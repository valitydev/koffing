...
export class CreateShopService {
    ...
    public getBankAccountPossibility(
        bankAccountSource: any
    ): true | { summary: string; detail: string } {
        ...
        if (!bankAccount.iban) {
            ...
            if (
                ...
                !(
                    bankDetails.bic ||
                    bankDetails.abartn ||
                    (bankDetails.name && bankDetails.countryCode && bankDetails.address)
                )
            ) {
                return {
                    summary: 'В данных международной банковской организации не заполнены все поля',
                    detail:
                        'Необходимо заполнить либо IBAN, либо BIC, либо ABA RTN, либо наименование, страну и адрес'
                };
            }
        }
        return true;
    ...
    private handleGroups() {
        this.handleStatus(this.contractForm, () => {
            ...
            this.contractFormService
                .toContractCreation(contractFormData, this.type)
        ...
        this.handleStatus(this.shopForm, () => {
            this.changeSet[ShopCreationStep.shop] = this.shopFormService.toShopCreation(
                this.contractID,
                this.payoutToolID,
                this.shopForm
            );
    ...
    private getNonresidentBankAccount({
        correspondentBankAccount: correspondentBankAccountSrc,
        ...bankAccount
    }: any): any {
        const correspondentBankAccount = this.getNonresidentBankAccountPart(
            correspondentBankAccountSrc
        );
        return {

    ...
    public getModificationType(modifications: PartyModification[]): string {
        ...
        modifications.forEach((modification: any) => {
            if (modification.shopModificationType === MODIFICATION_TYPE.ShopCreation) {
                ...
                modification.shopModificationType === MODIFICATION_TYPE.ShopContractBinding
            ) {
                const hasCreationContract = Boolean(
                    modifications.find(
                        (item: any) =>
                            item.contractModificationType === MODIFICATION_TYPE.ContractCreation
                    )
                );
                const hasCreationPayoutTool = Boolean(
                    modifications.find(
                        (item: any) =>
                            item.contractModificationType ===
                            MODIFICATION_TYPE.ContractPayoutToolCreation
                    )
                );
